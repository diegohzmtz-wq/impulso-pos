...
export default function Ticket({
  ...
  const precioItem = (item: any) => {
    const precioBase = Number(
      item.varianteSeleccionada?.precio || item.precio || 0
    );

    const extras = (item.modificadoresSeleccionados || []).reduce(
      (acc: number, mod: any) => acc + Number(mod.precioExtra || 0),
      0
    );
  ...
  return (
    <div className="flex h-full min-h-screen flex-col bg-white text-gray-950">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        {carrito.length === 0 ? (
          <div className="rounded-[28px] border border-dashed border-gray-300 p-8 text-center">
            <p className="text-lg font-black text-gray-900">Ticket vacío</p>
          ...
          carrito.map((item: any, index) => {
            const precioUnitario = precioItem(item);
                  ...
                  <div>
                    ...
                    {item.varianteSeleccionada?.nombre && (
                      <p className="mt-1 text-xs font-bold text-green-600">
                        Variante: {item.varianteSeleccionada.nombre}
                      </p>
                    )}
                    ...
                    {item.modificadoresSeleccionados?.length > 0 && (
                      <div className="mt-2 space-y-1">
                        {item.modificadoresSeleccionados.map((mod: any) => (
                          <p
                            key={mod.id}
